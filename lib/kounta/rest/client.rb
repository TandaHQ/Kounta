	...
	module REST
		class Client
			...
			def perform(url_hash, request_method, options={})
				response = @conn.send(request_method.to_sym, "#{path_from_hash(url_hash)}.#{FORMAT.to_s}", options).body
				raise Kounta::Errors::APIError, response['error_description'] if response['error']
				response
			end
